...
class Model_Xfile extends Model_Base{
  ...
  public function editAction(){	  
    ...
    if ($this->isPost()){
      $posts = $_POST;
	  $cond = Zend_Validate::is($posts['title'], 'StringLength',array(1,999));
	  if (!$cond){
		  $this->error("标题错误", "标题长度不够");
	  }
	  $cond = Zend_Validate::is($posts['editor_content'], 'StringLength',array(1,50000));
	  if (!$cond){
  ...
  public function acceptAction(){
	  ...
	  if ($this->view->login->group_id == 1 or $this->view->login->group_id == 2)
	  {
		  ...
		  $ret = $cmsdb->query("select * from v9_category where siteid=1");
...
{  //
	//include 'D:/webfulian/phpcms/base.php';
	//'D:\wamp\apps\xiaoxibaosong\lib\Model'
